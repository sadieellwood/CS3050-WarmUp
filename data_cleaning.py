...
movies["release_date"] = pd.to_datetime(
    movies["release_date"], format="%m/%d/%Y", errors="coerce"
)
...
inSeries["belongs_to_collection"] = inSeries["belongs_to_collection"].apply(
    ast.literal_eval
)
...
movies_clean = pd.concat(
    [inSeries.sample(100, random_state=1), noSeries.sample(100, random_state=1)]
)
movies_clean.columns = ["series", "release_date", "title", "rating"]
# clean
movies_clean["series"] = movies_clean["series"].str.replace(
    r"[^A-Za-z0-9 ]", "", regex=True
)
movies_clean["title"] = movies_clean["title"].str.replace(
    r"[^A-Za-z0-9 ]", "", regex=True
)
